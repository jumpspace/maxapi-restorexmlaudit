...
function parseAndWriteXML(xmlData, index) {
  ...
  parser.parseString(xmlData, (err, result) => {
    if (err) {
      ...
      fs.writeFileSync(`abEntryOppUdf${index}.json`, jsonResult, 'utf-8');
      console.log(`XML document ${index} parsed and saved as abEntryOppUdf${index}.json`);
    }
  });
}

// Read the input XML file
fs.readFile('AbEntryOppUdfs_master.xml', 'utf-8', (err, data) => {
  if (err) {
