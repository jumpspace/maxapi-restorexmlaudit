...
function parseAndWriteXML(xmlData, index) {
  ...
  parser.parseString(xmlData, (err, result) => {
    if (err) {
      ...
      fs.writeFileSync(`abEntryCaseNote${index}.json`, jsonResult, 'utf-8');
      console.log(`XML document ${index} parsed and saved as abEntryCaseNote${index}.json`);
    }
  });
}

// Read the input XML file
fs.readFile('AbEntryCaseNotes_master.xml', 'utf-8', (err, data) => {
  if (err) {
