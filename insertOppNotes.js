...
for (var i = minfile; i <= maxfile; i++) {
    var filename = "abEntryOppNotes/abEntryOppNote" + i + ".json";
    parseNote(filename);
}


function parseOppNote(filename) {
	fs.readFile(filename, (error, data) => {
		...
		let oldOppId = (typeof fieldClientId === 'object') ? "" : fieldClientId;
...
function getRecord(filename, userKey, note) {
	...
	fetch(oppFind, { method: "POST", body: JSON.stringify(request), headers: connectHeaders })
		.then((response) => response.json())
		.then((data) => {
			if (data.Code >= 0) {
				if (data.Opportunity.Data.length >= 1) {
					console.log(data.Opportunity.Data[0].Key + ": Record Key Found");
					console.log(filename + "-" + data.Opportunity.Data[0].Key);
					updateNoteFields(userKey, note);
					createNote(data.Opportunity.Data[0].Key, note);
				} else {
					console.log(note.oldOppId +  " - Record does not exist.");
				}
			} else {
