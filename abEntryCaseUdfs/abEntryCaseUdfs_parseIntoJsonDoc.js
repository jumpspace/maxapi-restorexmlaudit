...
function parseAndWriteXML(xmlData, index) {
  ...
  parser.parseString(xmlData, (err, result) => {
    if (err) {
      ...
      fs.writeFileSync(`abEntryCaseUdf${index}.json`, jsonResult, 'utf-8');
      console.log(`XML document ${index} parsed and saved as abEntryCaseUdf${index}.json`);
    }
  });
}

// Read the input XML file
fs.readFile('AbEntryCaseUdfs_master.xml', 'utf-8', (err, data) => {
  if (err) {
