...
function parseAndWriteXML(xmlData, index) {
  ...
  parser.parseString(xmlData, (err, result) => {
    if (err) {
      ...
      fs.writeFileSync(`abEntry${index}.json`, jsonResult, 'utf-8');
      console.log(`XML document ${index} parsed and saved as abEntry${index}.json`);
    }
  });
}

// Read the input XML file
fs.readFile('AbEntry_master.xml', 'utf-8', (err, data) => {
  if (err) {
